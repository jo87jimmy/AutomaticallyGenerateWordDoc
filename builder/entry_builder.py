...
class WordEntryBuilder:
    ...
    def build(self, word: str) -> Dict[str, Any]:
        ...
        """
        word = word.lower().strip()  # 確保輸入單字為小寫且無多餘空白，維持資料比對的一致性
        ...
        if not meanings:
            ...
            for m in wordnet_data:
                meanings.append({
                    ...
                    "synonyms": m["synonyms"],
                    "examples": examples[:1] if examples else [],  # 只有在有例句時才加入，避免空值錯誤
                })
        ...
        # 4.1 備援方案：若 Wiktionary 沒抓到，嘗試從 WordNet 抓衍生詞
        if not derivatives:
            derivatives = get_wordnet_derivatives(word)
            
        # 4.2 備援方案：若 Wiktionary 沒抓到，嘗試從 Datamuse API 抓片語
        if not phrases:
            phrases = fetch_phrases_from_api(word)
        
        # 4.3 備援方案：若 Wiktionary 沒抓到字源，嘗試從 Dictionary API 的 origin 欄位抓取
        if not etymology and api_data and api_data.get("origin"):
            etymology = [{"source": "Dictionary API", "description": api_data.get("origin")}]
        ...
        return {
            ...
            "phonetics": phonetics,
            "meanings": meanings,
            "phrases": phrases[:2],      # 限制最多兩筆片語
            "derivatives": derivatives[:2], # 限制最多兩筆衍生詞
